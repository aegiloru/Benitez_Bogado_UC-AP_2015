...
float MS561101BA::getPressure(uint8_t OSR, uint64_t rawPress) {
  ...
  int64_t TEMP= 2000 + (((int64_t)dT * (int64_t)_C[5]) >> 23);
  if(rawPress == NULL) {
    return NULL;
  }
  ...
  if(TEMP < 2000) // if temperature lower than 20 Celsius 
 {
  ...
  return (float)(( ((rawPress * SENS ) >> 21) - OFF) >> 15) *0.01;
//return -3;
}
...
int32_t MS561101BA::getDeltaTemp(uint8_t OSR) {
  uint64_t rawTemp = rawTemperature(OSR);
  if(rawTemp != NULL) {
...
float MS561101BA::getAltitude(float press,float sea_press, float temp) {
  //return (1.0f - pow(press/101325.0f, 0.190295f)) * 4433000.0f;
  if (press!=0){
  return ((1-pow((press / 1013.25), 0.19026)) * (288.15)) / 0.000065;
  } 
}
